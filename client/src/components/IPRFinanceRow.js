...
import { updateIprAllocateById } from '../actions/ipr';
import { getIPR } from '../actions/ipr';
import formatDate from '../utils/formatDate';

export default function IPRFinanceRow(props) {
  ...
  const handleAllocate = (id) => {
    dispatch(updateIprAllocateById(id));
  };
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{(row.approvalStage > 0) && formatDate(row.allocationDate)}</TableCell>
        <TableCell align="center">
          <Button sx={{ borderRadius: '20px' }} onClick={() => handleAllocate(row._id)} disabled={row.approvalStage > 0 ? true : false} variant="contained">Allocate</Button>
        </TableCell>
        {row.approvalStage > 0 ? (<TableCell align="center" sx={{ color: '#65C466' }}>Completed</TableCell>) : (<TableCell align="center" sx={{ color: '#c55615' }}>Pending</TableCell>)}
