...
import { updateIprFinalApproveById, getIPR } from '../actions/ipr';
import formatDate from '../utils/formatDate';

export default function IPRCEORow(props) {
  ...
  const handleCheckChange = (id, event) => {
    dispatch(updateIprFinalApproveById(id, event.target.checked));
  };
    ...
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">
          <Switch
            checked={row.approvalStage > 3}
            onChange={(event) => handleCheckChange(row._id, event)}
            inputProps={{ 'aria-label': 'controlled' }}
          />
        </TableCell>
        {row.approvalStage > 3 ? (<TableCell align="center" sx={{ color: '#65C466' }}>Approved</TableCell>) : (<TableCell align="center" sx={{ color: '#c55615' }}>Unapproved</TableCell>)}
